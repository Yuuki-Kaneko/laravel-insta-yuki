...
@section('content')
    ...
    <form action="{{ route('admin.users') }}" method="get" style="width: 10rem;" class="ms-auto my-3">
        <input type="text" name="search" id="search" value="{{ $search }}" placeholder="Search..." class="form-control form-control-sm">
    </form>

    <table class="table table-hover align-middle text-secondary bg-white border">
        <thead class="table-primary text-uppercasse small text-secondary">
            <th></th>
            <th>Name</th>
            <th>Email</th>
            <th>Created At</th>
                    ...
                    @if($user->id != Auth::user()->id)
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu">
                                <button class="dropdown-item text-danger" data-bs-toggle="modal" data-bs-target="#deactive-user{{ $user->id }}">
                                    <i class="fa-solid fa-user-slash"></i> Deactivete {{ $user->name }}
                                </button>
                            </div>
                            @else
                                <div class="dropdown-menu">
