    ...
    <div class="row justify-content-center">
        <div class="col-8">
            <form action="{{ route('profile.update') }}" method="post" class="rounded-3 shadow bg-white p-5" enctype='multipart/form-data'>
                ...
                <div class="row mb-3">
                    <div class="col-4">
                        ...
                        @else
                        <i class="fa-solid fa-circle-user text-secondary icon-lg d-block mx-auto"></i>
                        @endif
                    </div>
                ...
                <textarea name="introduction" id="introduction" cols="3" class="form-control">{{ old('introduction',Auth::user()->introduction) }}</textarea>
                @error('introduction')
                    <span class="text-danger small d-block">{{ $message }}</span>
                @enderror
            ...
            <form action="{{ route('profile.updatePassword') }}" method="post" class="rounded-3 shadow bg-white p-5 mt-5">
                ...
                <label for="new-password" class="form-label mt-3">Confirm New Password</label>
